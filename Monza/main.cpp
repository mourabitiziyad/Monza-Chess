...
Bitboard get_occupancy_set(int idx, int bits, Bitboard attacks)
{
  ...
  for (int x = 0; x < bits; x++)
  {
    ...
    if (idx & (1 << x))
      occupancy |= (1ULL << square); // What are we checking for here?
  }
  return occupancy;
}
...
Bitboard mask_king_attacks(int square)
{
  ...
  if (bitboard >> 8)
    attacks |= (bitboard >> 8); // Condition may not be needed
...
static inline int is_square_attacked(int square, int side)
{
  if (side == white)
  {
    return ((pawn_attack_moves[black][square] & piece_bitboards[P]) | (knight_attack_moves[square] & piece_bitboards[N]) | (king_attack_moves[square] & piece_bitboards[K]) | (get_bishop_attacks(square, piece_occupancy[both]) & piece_bitboards[B]) | (get_rook_attacks(square, piece_occupancy[both]) & piece_bitboards[R]) | (get_queen_attacks(square, piece_occupancy[both]) & piece_bitboards[Q])) ? 1 : 0;
  }
  else if (side == black)
  {
    return ((pawn_attack_moves[white][square] & piece_bitboards[p]) | (knight_attack_moves[square] & piece_bitboards[n]) | (king_attack_moves[square] & piece_bitboards[k]) | (get_bishop_attacks(square, piece_occupancy[both]) & piece_bitboards[b]) | (get_rook_attacks(square, piece_occupancy[both]) & piece_bitboards[r]) | (get_queen_attacks(square, piece_occupancy[both]) & piece_bitboards[q])) ? 1 : 0;
  }
  return 0;
}
        ...
        while (bitboard_copy)
        {
          ...
          if (!(target < a8) && !bit_on_square(piece_occupancy[both], target))
          {
            ...
            else
            {
              ...
              if ((source >= a2 && source <= h2) && !bit_on_square(piece_occupancy[both], (target - 8)))
              {
      ...
      if (piece == p)
      {
        while (bitboard_copy)
        {
          ...
          if (!(target > h1) && !bit_on_square(piece_occupancy[both], target))
          {
            ...
            {
              add_move(move_list, encode_move(source, target, piece, 0, 0, 0, 0, 0));
              if ((source >= a7 && source <= h7) && !bit_on_square(piece_occupancy[both], (target + 8)))
              {
                add_move(move_list, encode_move(source, (target + 8), piece, 0, 0, 1, 0, 0));
              }
            }
...
void display_move_list(moves *move_list)
{
  ...
  for (int count = 0; count < move_list->count; count++)
  {
    printf("%s%s%c      %s           %d               %d                   %d                 %d\n",
           ...
           get_move_promoted(move_list->moves[count]) ? find_promoted_piece_index(get_move_promoted(move_list->moves[count]))
                                                      : ' ',
...
static inline void perft_driver(int depth)
{
  ...
  {
    copy();
    if (!make_move(move_list->moves[moves], all))
      continue;
  ...
  for (int bb_piece = P; bb_piece <= k; bb_piece++)
  {
    ...
    while (board_copy)
    {
      ...
      case P:
        ...
        if (count_bits(piece_bitboards[P] & file_masks[square]) > 1)
          score += count_bits(piece_bitboards[P] & file_masks[square]) * double_pawn_penalty;
        if (!(piece_bitboards[P] & isolated_pawn_masks[square]))
          score += isolated_pawn_penalty;
        if (!(piece_bitboards[P] & white_passed_pawn_masks[square]))
      ...
      case R:
        ...
        if (!(piece_bitboards[P] & file_masks[square]))
          score += semi_open_file_score;
        if (!((piece_bitboards[P] | piece_bitboards[p]) & file_masks[square]))
      ...
      case p:
        ...
        if (!(piece_bitboards[p] & isolated_pawn_masks[square]))
          score -= isolated_pawn_penalty;
        if (!(piece_bitboards[p] & black_passed_pawn_masks[square]))
...
int negamax(int alpha, int beta, int depth)
{
  ...
  if (ply && (score = probe_hash(alpha, beta, depth)) != no_hash_entry)
    return score;
  if ((nodes & 2047) == 0)
    communicate();
  pv_length[ply] = ply;
